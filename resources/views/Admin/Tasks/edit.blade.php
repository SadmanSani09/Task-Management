...
<div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm p-8">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500">Task ID: #{{ $task->id }} — Created {{ $task->created_at->diffForHumans() }}</p>
        </div>
        ...
        <div class="space-y-5">
                ...
                <div>
                    ...
                    <select name="status" class="mt-1 w-full border-gray-300 rounded-lg text-sm">
                        @foreach(['pending','in_progress','completed','overdue'] as $s)
                            <option value="{{ $s }}" @selected(old('status',$task->status)==$s)>{{ str_replace('_',' ',ucfirst($s)) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="border-t pt-5">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    Assigned Workers <span class="text-xs text-gray-400">(check to assign, uncheck to remove)</span>
                </label>
