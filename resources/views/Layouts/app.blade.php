        ...
        <nav class="flex-1 p-4 space-y-1">
            <a href="{{ route('dashboard') }}"
               class="flex items-center px-4 py-3 rounded-lg transition
                ...
                <i class="fas fa-th-large w-5"></i><span class="ml-2">Dashboard</span>
            </a>
            @if(auth()->user()->isAdmin())
                <a href="{{ route('tasks.create') }}"
                   class="flex items-center px-4 py-3 rounded-lg transition
                          {{ request()->routeIs('tasks.create') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100' }}">
                    <i class="fas fa-plus-circle w-5"></i><span class="ml-2">New Task</span>
                </a>
                <a href="{{ route('categories.index') }}"
                   class="flex items-center px-4 py-3 rounded-lg transition
                          {{ request()->routeIs('categories.*') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100' }}">
                    <i class="fas fa-tags w-5"></i><span class="ml-2">Categories</span>
                </a>
                <a href="{{ route('users.index') }}"
                   class="flex items-center px-4 py-3 rounded-lg transition
                          {{ request()->routeIs('users.*') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100' }}">
                    <i class="fas fa-users w-5"></i><span class="ml-2">Workers</span>
                </a>
            @endif
        </nav>
        <div class="p-4 border-t">
